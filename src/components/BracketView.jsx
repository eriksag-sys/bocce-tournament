...
const { CARD, BORDER, GREEN, YELLOW, BLUE, RED, MUTED, LIGHT } = COLORS;
...
function BracketSlot({ game, style, name, seedFor, onClick, isChamp, isAdmin }) {
    ...
    return (
        <div style={{
            ...style,
            background: isChamp ? '#181000' : done ? '#091a0d' : active ? '#091422' : CARD,
            ...
            <div style={{
                background: 'rgba(0,0,0,.4)', padding: '3px 8px', display: 'flex',
            ...
            {[
                ...
                <div key={i} style={{
                    ...
                    background: win ? GREEN + '11' : 'transparent'
                }}>
                    {seed && (
...
export default function BracketView({ S, name, setS, isAdmin }) {
    ...
    const getTop = (round, slot) => {
        ...
        if (round === 4) return slot === 0 ? 3 * SLOT_H - GAME_H / 2 : 5 * SLOT_H - GAME_H / 2;
        return 0;
    };
        ...
        <div>
            {champion && (
                <div style={{
                    ...
                    background: '#1a1200', border: `2px solid ${YELLOW}`, borderRadius: 8
                }}>
            ...
            <div style={{ overflowX: 'auto', paddingBottom: 12 }}>
                <div style={{ position: 'relative', height: totalH + 32, width: totalW, minWidth: totalW }}>
                    ...
                    <svg style={{ position: 'absolute', top: 18, left: 0, width: totalW, height: totalH, pointerEvents: 'none' }}>
                        ...
                        {sf.map((g, i) => (
                            <path key={g.id} d={connLine(3, i, 4, i)} fill="none" stroke={BORDER} strokeWidth={1.5} />
                        ))}
                    </svg>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {bracketSeeds.map(s => (
                        <div key={s.pid} style={{
                            ...
                            <span style={{ color: '#fff', fontWeight: 600 }}>{name(s.pid)}</span>
                            <span style={{
